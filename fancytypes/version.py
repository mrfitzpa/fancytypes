...
def _get_version_summary():
    """Get version summary of ``fancytypes``.
    ...
    -------
    summary : `str`
        Version summary of ``fancytypes``.
    """
    # Check versions of fancytypes.
    from . import _version
    if _version.version != version:
        raise ValueError("fancytypes version has changed since "
                         "installation/compilation")



    # Generate summary.
    summary = ("fancytypes {fancytypes_ver!s};\n"
               "git revision {git_rev!s} using\n"
               "python {python_ver!s}")
    
    summary = summary.format(fancytypes_ver=full_version,
